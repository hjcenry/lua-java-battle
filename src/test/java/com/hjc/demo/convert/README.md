...
|fileDir|lua文件目录，以BattleLogic/Lua/为根路径开始|工具类同目录下：~/src/test/java/lua/|
|comment|注释|无|
...
function ServerLib:ctor()
    self.battle = luajava.bindClass("com.hjc.socket.logic.raid.lua.LuaBattleFunction")
    self.propertyTool = luajava.bindClass("com.hjc.socket.logic.raid.lua.LuaPropertyTool")
    self.logTool = luajava.bindClass("com.hjc.luaj.log.LuaLogTool")
    self.timeService = luajava.bindClass("com.hjc.service.TimeService")
end
```
